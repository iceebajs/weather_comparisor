...
[~,night_table,~] = xlsread([pwd, '/образец.xlsx'], 1,'A2:AF67');
[~,day_table,~] = xlsread([pwd, '/образец.xlsx'], 1,'AH2:BM67');
night_table(3,:) = [];
day_table(3,:) = [];
night_table(:,8) = [];
night_table(:,5) = [];
day_table(:,8) = [];
day_table(:,5) = [];
day_table(2,15) = day_table(2,27);
night_table(2,15) = night_table(2,27);
